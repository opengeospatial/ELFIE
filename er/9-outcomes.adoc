...
* Keeping the client as generic as possible, allows vizualization of not only Surface-ground water networks interaction Use Case related JSON-LD files but also, those from the other Use Cases.
* Such an approach should proves valuable for demonstration of what linked-data is using domain-scientists own datasets as well as how it can help relate to other domains without having to modify source data.
